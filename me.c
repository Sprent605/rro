...
int main(void)
{
	int ch, days, H, M, num, passin, fun,y,m,d,skip;
	char user[999];
	char pass[999];
	char pas[999];
	char exp[999];
...
choice:
	...
	exp:
		printf("\033[1;36m\nEnter expiry in days in numbers: \033[0m");
		num = scanf("%d", &days);
		if (!num)
		{
...
pfz=fopen("day","r");
    ...
    char bufferc[30];
    strftime(bufferc, 30, "echo \"%Y-%m-%d\">ery", &t);
    system(bufferc);
...
pfh=fopen("ery","r");
		...
		char dats[50];
		snprintf(dats,50,"sudo chage -E %s %s",dat,user);
		system(dats);
